import React from 'react'

const MiddleThird = () => {
  return (
    <div>
<div class="container top-0 md:mt-[100px] px-6 mx-auto">
  
  <section class="mb-32 text-[#2f2e41] text-center">
   
    <h2 class="text-3xl font-bold mb-10">Trusted by <u class="">2,000,000+</u> users</h2>

    <div class="grid md:grid-cols-2 lg:grid-cols-4 gap-2 items-center">
      <div class="mb-12 lg:mb-0">
        <img
          src="https://mdbootstrap.com/img/Photos/new-templates/landing-page/2.png"
          class="img-fluid grayscale px-6 md:px-12"
          alt="Nasa - logo"
        />
      </div>

      <div class="mb-12 lg:mb-0">
        <img
          src="https://mdbootstrap.com/img/Photos/new-templates/landing-page/1.png"
          class="img-fluid grayscale px-6 md:px-12"
          alt="Amazon - logo"
        />
      </div>

      <div class="mb-12 lg:mb-0">
        <img
          src="https://mdbootstrap.com/img/Photos/new-templates/landing-page/3.png"
          class="img-fluid grayscale px-6 md:px-12"
          alt="Nike - logo"
        />
      </div>

      <div class="mb-12 lg:mb-0">
        <img
          src="https://mdbootstrap.com/img/Photos/new-templates/landing-page/4.png"
          class="img-fluid grayscale px-6 md:px-12"
          alt="Ikea - logo"
        />
      </div>
    </div>
  </section>
  
</div>
    </div>
  )
}

export default MiddleThird